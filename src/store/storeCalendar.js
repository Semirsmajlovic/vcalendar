...
const storeCalendar = {
	...
	actions: {
		...
		async updateShift({ commit, state, getters, dispatch }, payload) {
			let docRef;
			let index; // Declare index once at the top
			try {
				if (!payload.isRecurring) {
					if (!payload.actionType) {
						...
						if (exceptionIndex !== -1) {
							...
							commit('UPDATE_EXCEPTION', { index: exceptionIndex, payload });
						}
					} else {
						// Update recurring event to one-time event.
						let divergedIndex = getters.getIndexExceptionDiverged(payload);
						if (divergedIndex !== -1) {
							docRef = doc(db, "exceptions", payload.id);
							await updateDoc(docRef, payload);
							commit('UPDATE_EXCEPTION', { index: divergedIndex, payload });
						}
					}
				} else {
					switch (payload.actionType.description) {
						case 'updateAll': {
							index = getters.getIndexEvent(payload); // Reassign index as needed
							if (index !== -1) {
						...
						case 'updateInstance': {
							try {
								let eventIndex = getters.getIndexEvent(payload);
								if (eventIndex !== -1) {
									let originalEvent = state.events[eventIndex];
									let exceptionPayload = {
										...payload,
										isRecurring: false,
										originalEventId: originalEvent.id,
									};
									const exceptionDocRef = await addDoc(collection(db, "exceptions"), exceptionPayload);
									exceptionPayload.id = exceptionDocRef.id;
									commit('ADD_EXCEPTION', exceptionPayload);
								}
							} catch (e) {
								console.error(`[storeCalendar.js/updateInstance]: Error with ${e}`);
								dispatch('updateSnackMessage', `Error with ${e}`, { root: true });
							}
							break;
						}
						...
						default:
							console.warn(`Unknown actionType in updateShift: ${payload.actionType.description}`);
							dispatch('updateSnackMessage', `Unknown actionType in updateShift`, { root: true });
					}
				}
			} catch (e) {
				console.error(`[storeCalendar.js/updateShift]: Error with ${e}`);
				dispatch('updateSnackMessage', `Error with ${e}`, { root: true });
			}
		},
